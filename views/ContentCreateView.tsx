      ...
      <form onSubmit={handleSave} className="space-y-3 rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-3 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-4 lg:p-5">
        <div className="flex items-center gap-2.5 border-b border-slate-200/80 pb-2.5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Content Details</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {!isFollowMode && (
            <>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div ref={typePickerWrapRef} className={`relative ${panelClass}`}>
                  <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Content Type</p>
                  <button
                    type="button"
                    onClick={() => {
                      setShowDatePicker(false);
                ...
                <div ref={datePickerWrapRef} className={`relative ${panelClass}`}>
                  <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Publish Date</p>
                  <button
                    type="button"
                    onClick={() => {
                      setShowTypePicker(false);
                  ...
                  {showDatePicker && (
                    <div className="absolute right-0 top-full z-30 -mt-px w-[280px] overflow-hidden rounded-b-[0.95rem] border border-slate-200 border-t-0 bg-white p-2 shadow-[0_16px_36px_rgba(15,23,42,0.10)]">
                      <div className="rounded-[0.9rem] border border-violet-100 bg-[linear-gradient(180deg,rgba(245,243,255,0.95),rgba(255,255,255,0.94))] px-3 py-2">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-600">Content Calendar</p>
                            <p className="mt-0.5 text-[15px] font-semibold text-slate-900">{formatMonthLabel(calendarMonth)}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => setCalendarMonth((prev) => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}
                              className="flex h-8 w-8 items-center justify-center rounded-[0.9rem] border border-violet-100 bg-white text-slate-600 transition hover:border-violet-200 hover:text-violet-700"
                      ...
                      <div className="mt-2.5 grid grid-cols-7 gap-1.5 px-1 text-center text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">
                        ...
                        {calendarDays.map((day) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex h-8 items-center justify-center rounded-[0.8rem] text-[13px] font-medium transition ${
                                isSelected
                      ...
                      <div className="mt-2.5 flex items-center justify-between border-t border-slate-100 px-1 pt-2.5">
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="rounded-[0.85rem] border border-violet-200 bg-violet-50/80 px-2.5 py-1.5 text-[11px] font-medium text-violet-700 transition hover:bg-violet-100"
                        >
                          Use Today
                        </button>
                        <button
                          ...
                          className="rounded-[0.85rem] px-2.5 py-1.5 text-[11px] font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"
                        >
                          Close
                        </button>
                      </div>
              ...
              {fieldErrors.contentDate ? <p className="-mt-0.5 text-xs text-red-600">{fieldErrors.contentDate}</p> : null}
            </>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              className={`min-h-[92px] ${inputBaseClass} resize-y px-3.5 py-3 leading-7 ${fieldErrors.description ? 'border-red-200 focus:border-red-300 focus:ring-4 focus:ring-red-100' : 'border-slate-200 focus:border-violet-300 focus:ring-4 focus:ring-violet-100'}`}
            />
            <div className="mt-1.5 flex items-center justify-between gap-3">
