  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-brand-red/8 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-brand-red">
            <CalendarDays size={13} />
            Calendar view
          </div>
          <h3 className="mt-2.5 text-[1.35rem] font-semibold text-slate-900 tracking-tight">{monthLabel}</h3>
        </div>
      ...
      <div className="mt-4 grid grid-cols-7 gap-1.5">
        {weekLabels.map((label) => (
          <div key={label} className="px-1 text-center text-[10px] font-semibold uppercase tracking-wide text-slate-400">
            {label}
          </div>
        ))}
        ...
        {calendarDays.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative h-10 rounded-lg border text-[0.95rem] font-semibold transition-all ${toneClass}`}
            >
              {item.label}
        ...
        {activeDateLeaves.length > 0 ? (
          <div className="mt-3 space-y-2.5">
            {activeDateLeaves.map((leave) => (
              <div key={`${activeDateKey}-${leave._id}`} className="rounded-xl border border-white bg-white px-3 py-2.5 shadow-sm">
