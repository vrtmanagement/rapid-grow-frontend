...
const LeaveCalendarView: React.FC<Props> = ({
  ...
  return (
    <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-[0_18px_42px_rgba(15,23,42,0.08)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-brand-red/8 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-brand-red">
            <CalendarDays size={13} />
            Calendar view
          </div>
          <h3 className="mt-2.5 text-[1.35rem] font-semibold text-slate-900">{monthLabel}</h3>
        </div>
      ...
      <div className="mt-4 grid grid-cols-7 gap-1.5">
        {weekLabels.map((label) => (
          <div key={label} className="px-1 text-center text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">
            {label}
          </div>
        ))}
        ...
        {calendarDays.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative h-10 rounded-xl border text-[0.95rem] font-semibold transition-all ${toneClass}`}
            >
              {item.label}
              {showEmployeeDetails && dayLeaves.length > 1 ? (
                <span className="absolute right-1 top-1 inline-flex min-w-[0.95rem] items-center justify-center rounded-full bg-slate-900/80 px-1 text-[8px] font-semibold text-white">
                  {dayLeaves.length}
                </span>
              ) : null}
              {(hasApproved || hasPending || hasRejected || isSelectedRange) ? (
                <span className="absolute bottom-1 left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-current opacity-80" />
              ) : null}
            </button>
          );
        })}
      ...
      <div className="mt-4 rounded-[20px] border border-slate-200 bg-slate-50/80 p-3.5">
        ...
        {activeDateLeaves.length > 0 ? (
          <div className="mt-3 space-y-2.5">
            {activeDateLeaves.map((leave) => (
              <div key={`${activeDateKey}-${leave._id}`} className="rounded-[18px] border border-white bg-white px-3 py-2.5 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900">{getLeaveTypeLabel(leave.type)}</p>
                    {showEmployeeDetails ? (
                      <p className="mt-1 truncate text-[10px] font-semibold text-slate-500">{getEmployeeLabel(leave)}</p>
                    ) : null}
                  </div>
                ...
                <p className="mt-1.5 text-xs leading-5 text-slate-500">{leave.reason || 'No reason added'}</p>
              </div>
            ))}
          </div>
        ) : (
