...
const LeaveDetailModal: React.FC<LeaveDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
      <div className={`w-full max-w-xl rounded-xl border p-6 shadow-sm ${leaveDetailStatusThemes[selectedDetailLeave.status].shell}`}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Leave details</p>
            <h3 className="mt-2 text-[1.9rem] font-semibold tracking-[-0.03em] text-slate-950">{getLeaveTypeLabel(selectedDetailLeave.type)}</h3>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-slate-200 bg-white/90 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
          >
            Close
          </button>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className={`rounded-xl border p-4 ${leaveDetailStatusThemes[selectedDetailLeave.status].panel}`}>
          ...
          <div className={`rounded-xl border p-4 ${leaveDetailStatusThemes[selectedDetailLeave.status].panel}`}>
            ...
            <div className="mt-3">
              <span className={`inline-flex rounded-full px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide ${leaveDetailStatusThemes[selectedDetailLeave.status].badge}`}>
        ...
        {isApproverPortal && getEmployeeRecordLabel(selectedDetailLeave) ? (
          <div className="mt-4">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Employee</p>
        ...
        {viewerRole === 'employee' && selectedDetailLeave.status !== 'PENDING' && formatDecisionRole(selectedDetailLeave.decidedByRole) ? (
          <div className="mt-4">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Handled by</p>
            <p className="mt-1 inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-slate-900 shadow-sm">
        ...
        <div className={`mt-5 rounded-xl border p-4 ${leaveDetailStatusThemes[selectedDetailLeave.status].panel}`}>
